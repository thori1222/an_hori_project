...
int speed_mode = 0;  //0: joystick, 1: handle and pedal
...
void joy_callback(const sensor_msgs::Joy& joy_msg)
{
    if (speed_mode == 0)
    {
        ...
        if(joy_msg.buttons[0] == 1){
            if(joy_y_e_control < 0.35)
                joy_y_e_control += 0.35;
        }

        if(joy_msg.buttons[2] == 1){
            if(joy_y_e_control > 0.1)
                joy_y_e_control += -0.35;
        }
    }
    ...
    if (speed_mode == 1)
    {
        if(joy_msg.axes[1] > 0.3)
            if(joy_y_e_control < 0.35)
                joy_y_e_control += 0.35;

        joy_speed_control = 1 + joy_msg.axes[2];
    }
}
